...
{
  n<-dim(X)[1] ; p<-dim(X)[2] #Guarda las dimensiones de la matriz X
  if(p==0) { s20<-mean(y^2) } #Si no hay columnas,s20 es mean
  H0<-0 ; if(p>0) { H0<- (g/(g+1)) * X%*%solve(t(X)%*%X)%*%t(X) }#Aquí encuentra la matriz Hat
  SS0<- t(y)%*%( diag(1,nrow=n)  - H0 ) %*%y #Aquí encuentra la suma de cuadrados del error
